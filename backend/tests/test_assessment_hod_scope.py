"""
VCIS 3.0 — Assessment Directory HOD Department Scope Security Tests

Validates:
1. HOD with department 1 sees Assessments for students in department 1.
2. HOD with department 1 does NOT see Assessments for students in department 2.
3. HOD with department 2 sees Assessments for students in department 2.
4. HOD with multiple students and multiple Assessment records receives only records from their department.
...
11. HOD cannot bypass scope using semester/assessment filters (e.g. ?assessment_type=ct1, ?semester=1).
12. ADMIN retains institutional/global Assessment directory access (all departments).
13. FACULTY retains existing institutional/global Assessment directory access (all departments).
14. STUDENT remains denied from the general Assessment directory -> 403 Forbidden.
15. Unauthenticated request remains 401 Unauthorized.
16. Existing Assessment query/filter behavior continues to work within HOD scope.
17. Existing Assessment detail endpoint behavior remains unchanged across roles.
18. Direct security test: HOD cannot retrieve an outside-department Assessment through any parameter combination.
"""
...
os.environ["JWT_SECRET_KEY"] = "test-assessment-hod-scope-secret-key-12345"
...
class TestAssessmentHodScope(unittest.TestCase):
    """Test suite verifying department-scoped assessment directory access for HOD users."""
    ...
    def setUp(self):
        ...
        # 6. Assessment records across Departments
        # Dept 1 Assessments
        self.ass_d1_a = Assessment(
            id=1,
        ...
        self.ass_d2_a = Assessment(
            ...
            obtained_marks=42,
            assessment_date=date(2023, 10, 15),
            remarks="Charlie very good",
        )
        self.ass_d2_b = Assessment(
            id=4,
            enrollment_id=self.enrollment_d2_b.id,
            assessment_type=AssessmentType.CT2,
            assessment_name="Cycle Test 2",
            max_marks=50,
            obtained_marks=40,
            assessment_date=date(2023, 11, 15),
            remarks="Diana good",
        )
        self.db.add_all([
            self.ass_d1_a,
    ...
    def test_01_hod_dept1_sees_only_dept1_assessments(self):
        """1. HOD with department 1 sees Assessments for students in department 1."""
        status, data = self._get("/api/v1/assessments/", user=self.user_hod_dept1)
        self.assertEqual(status, 200)
        self.assertIsInstance(data, list)
        self.assertEqual(len(data), 2)
        ass_ids = {a["id"] for a in data}
        self.assertEqual(ass_ids, {1, 2})
    ...
    def test_02_hod_dept1_does_not_see_dept2_assessments(self):
        """2. HOD with department 1 does NOT see Assessments for students in department 2."""
        status, data = self._get("/api/v1/assessments/", user=self.user_hod_dept1)
        self.assertEqual(status, 200)
        ass_ids = {a["id"] for a in data}
        self.assertNotIn(3, ass_ids)
        self.assertNotIn(4, ass_ids)
        enrollment_ids = {a["enrollment_id"] for a in data}
        self.assertNotIn(self.enrollment_d2_a.id, enrollment_ids)
        self.assertNotIn(self.enrollment_d2_b.id, enrollment_ids)

    def test_03_hod_dept2_sees_only_dept2_assessments(self):
        """3. HOD with department 2 sees Assessments for students in department 2."""
        status, data = self._get("/api/v1/assessments/", user=self.user_hod_dept2)
        self.assertEqual(status, 200)
        self.assertIsInstance(data, list)
        self.assertEqual(len(data), 2)
        ass_ids = {a["id"] for a in data}
        self.assertEqual(ass_ids, {3, 4})
        enrollment_ids = {a["enrollment_id"] for a in data}
        self.assertEqual(enrollment_ids, {self.enrollment_d2_a.id, self.enrollment_d2_b.id})

    def test_04_hod_multiple_records_only_receives_department_assessments(self):
        """4. HOD with multiple students/records only receives records from their department."""
        status, data = self._get("/api/v1/assessments/", user=self.user_hod_dept1)
        self.assertEqual(status, 200)
        for a in data:
            # Trace Assessment -> Enrollment -> Student -> department_id
    ...
    def test_05_hod_with_no_faculty_profile_receives_403(self):
        """5. HOD with no Faculty profile receives 403 Forbidden."""
        status, data = self._get("/api/v1/assessments/", user=self.user_hod_orphan)
    ...
    def test_07_hod_cannot_bypass_scope_using_student_id_query_parameter(self):
        ...
        status, data = self._get(f"/api/v1/assessments/?student_id={self.student_d2_a.id}", user=self.user_hod_dept1)
        self.assertEqual(status, 200)
        # Still strictly returns dept 1 assessment records
        for a in data:
    ...
    def test_08_hod_cannot_bypass_scope_using_enrollment_id_query_parameter(self):
        ...
        status, data = self._get(f"/api/v1/assessments/?enrollment_id={self.enrollment_d2_a.id}", user=self.user_hod_dept1)
        self.assertEqual(status, 200)
        for a in data:
            self.assertIn(a["id"], [1, 2])
    ...
    def test_09_hod_cannot_bypass_scope_using_subject_id_query_parameter(self):
        """9. HOD cannot bypass scope using subject_id query parameters (e.g. ?subject_id=201)."""
        status, data = self._get(f"/api/v1/assessments/?subject_id={self.subject_2.id}", user=self.user_hod_dept1)
        self.assertEqual(status, 200)
        for a in data:
            self.assertIn(a["id"], [1, 2])
    ...
    def test_10_hod_cannot_bypass_scope_using_course_id_query_parameter(self):
        """10. HOD cannot bypass scope using course_id query parameters (e.g. ?course_id=2)."""
        status, data = self._get(f"/api/v1/assessments/?course_id={self.course_2.id}", user=self.user_hod_dept1)
        self.assertEqual(status, 200)
        for a in data:
            self.assertIn(a["id"], [1, 2])

    def test_11_hod_cannot_bypass_scope_using_semester_assessment_filters(self):
        """11. HOD cannot bypass scope using semester/assessment filters (e.g. ?assessment_type=ct2, ?semester=1)."""
        status, data = self._get(
            f"/api/v1/assessments/?assessment_type=ct2&semester=1&department_id={self.dept_2.id}",
    ...
    def test_12_admin_retains_institutional_global_assessment_access(self):
        """12. ADMIN retains institutional/global Assessment access (all 4 records)."""
        status, data = self._get("/api/v1/assessments/", user=self.user_admin)
        self.assertEqual(status, 200)
        self.assertIsInstance(data, list)
        self.assertEqual(len(data), 4)
        ass_ids = {a["id"] for a in data}
        self.assertEqual(ass_ids, {1, 2, 3, 4})

    def test_13_faculty_retains_institutional_global_assessment_access(self):
        """13. FACULTY retains existing institutional/global Assessment access (all 4 records)."""
        status, data = self._get("/api/v1/assessments/", user=self.user_faculty)
        self.assertEqual(status, 200)
        self.assertIsInstance(data, list)
        self.assertEqual(len(data), 4)
        ass_ids = {a["id"] for a in data}
    ...
    def test_14_student_remains_denied_from_general_assessment_directory(self):
        """14. STUDENT remains denied from general Assessment directory -> 403 Forbidden."""
        status, data = self._get("/api/v1/assessments/", user=self.user_student_1)
        self.assertEqual(status, 403)
        self.assertIn("detail", data)

    def test_15_unauthenticated_request_remains_401(self):
        """15. Unauthenticated request remains 401 Unauthorized."""
        status, data = self._get("/api/v1/assessments/", user=None)
        self.assertEqual(status, 401)

    def test_16_existing_assessment_query_filters_work_within_hod_scope(self):
        """16. Existing Assessment query/filter behavior continues to work within HOD scope."""
        status, data = self._get(
            "/api/v1/assessments/?limit=10&offset=0",
            user=self.user_hod_dept1,
        )
        self.assertEqual(status, 200)
        self.assertEqual(len(data), 2)
    ...
    def test_17_assessment_detail_endpoint_behavior_remains_intact(self):
        """17. Existing Assessment detail endpoint behavior remains unchanged across roles."""
        # Admin accesses detail -> 200
        status_admin, data_admin = self._get(f"/api/v1/assessments/{self.ass_d1_a.id}", user=self.user_admin)
        self.assertEqual(status_admin, 200)
        self.assertEqual(data_admin["id"], self.ass_d1_a.id)

        # Faculty accesses detail -> 200
        status_fac, data_fac = self._get(f"/api/v1/assessments/{self.ass_d1_a.id}", user=self.user_faculty)
        self.assertEqual(status_fac, 200)
        self.assertEqual(data_fac["id"], self.ass_d1_a.id)

        # Student 1 accesses own assessment -> 200
        status_s1, data_s1 = self._get(f"/api/v1/assessments/{self.ass_d1_a.id}", user=self.user_student_1)
        self.assertEqual(status_s1, 200)
        self.assertEqual(data_s1["id"], self.ass_d1_a.id)

        # Student 1 accesses other student's assessment -> 403 Forbidden
        status_other, _ = self._get(f"/api/v1/assessments/{self.ass_d2_a.id}", user=self.user_student_1)
        self.assertEqual(status_other, 403)

    def test_18_security_hod_cannot_retrieve_outside_department_assessment(self):
        """18. Security Test: HOD cannot retrieve outside-department Assessment records via any parameter combination."""
        # Outside assessment record is id=3 (student_d2_a, enrollment_d2_a)
        status, data = self._get(
            f"/api/v1/assessments/?student_id={self.student_d2_a.id}&enrollment_id={self.enrollment_d2_a.id}&subject_id={self.subject_2.id}&department_id={self.dept_2.id}",
            user=self.user_hod_dept1,
        )
        self.assertEqual(status, 200)
        returned_ids = {a["id"] for a in data}
        self.assertNotIn(self.ass_d2_a.id, returned_ids)
        self.assertNotIn(self.ass_d2_b.id, returned_ids)
        for a in data:
            self.assertIn(a["id"], [1, 2])
